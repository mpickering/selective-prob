...
matchS :: (Eq a, Selective f, Lift a) => Cases a -> f a -> (a -> f b) -> f (Either a b)
matchS (Cases cs _) x f = foldr (\c -> eliminate c (f c)) ([|| Left ||]  <!$> x) cs

eliminate :: (Eq a, Selective f, Lift a) => a -> f b -> f (Either a b) -> f (Either a b)
...
bindS :: (Lift a, Bounded a, Enum a, Eq a, Selective f) => f a -> (a -> f b) -> f b
...
branch x l r = cfmap ([||  (fmap Left) ||]) x <*? cfmap ([|| (fmap Right)||]) l <*? r
